...
This repository is in **foundation**. Identity, documentation architecture, Cursor asset ownership, Cursor component architecture, agent operating model, skill operating model, rule operating model, adoption contract (copy/pin of a versioned subset), human-AI interaction practice, OpenSpec governance, and repository hygiene are in place.

The Cursor component constitution, agent operating model, skill operating model, rule operating model, and human-AI interaction practice exist. A skill, agent, command, or rule catalog does not. Prompt libraries and prompt template catalogs are not part of the framework. Those catalogs, a multi-agent implementation, and a project installer are later OpenSpec changes. Vendor OpenSpec Cursor assets are obtained from OpenSpec in each repository; they are not part of the copied SDD baseline.
